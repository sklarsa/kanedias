...
Once child sockets could bind, live validation observed a legitimate `starting` child snapshot and the harness immediately asserted Pi binding and model fields that are committed only after startup. Require the read and writer visibility predicates to match kind and context, carry non-empty Pi/model binding, and be in an active controllable lifecycle (`ready`, `running`, or `awaiting_handoff`) before running bound-state assertions or routed control calls. The child can transition from `ready` to `running` before a polling client observes the intermediate state, so requiring exactly `ready` is itself racy. Add a focused predicate regression that rejects matching but unbound `starting` and `running` snapshots, accepts bound active snapshots, and rejects terminal states. This preserves production lifecycle behavior and removes acceptance-only races.

Current `origin/main` treats `WorkspaceStart` as selection of an existing seed-volume checkout, not a request to clone a missing repository; a direct CLI root without bootstrap correctly starts at `/workspace`. The recursive acceptance's writer phase assumed its disposable repository already existed at an obsolete owner-qualified path, but that repository is intentionally absent from the seed. Keep manual roots at the default workspace, clone the preflighted disposable remote as the managed user into the root's run-local copied volume immediately before the writer phase, and perform checkout Git inspection under that same managed-user environment using the canonical checkout basename (`/workspace/repos/<repository-name>`). The forked child then inherits that exact disposable checkout without mutating the seed. This changes only acceptance setup and preserves production repository-start validation.
